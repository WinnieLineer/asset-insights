...
export function PortfolioCharts({ allocationData, historicalData, displayCurrency, language, loading, height }: any) {
  ...
  const trendHeight = height ? Math.max(200, height * 0.6) : 400;
  const allocationHeight = height ? Math.max(200, height * 0.3) : 300;

  return (
    <div className="space-y-8 h-full flex flex-col">
      <div className="modern-card p-6 border-slate-100 bg-white relative shadow-xl rounded-2xl flex flex-col flex-1">
        <div className="w-full mb-4 flex items-center justify-between">
          <h3 className="text-[10px] xl:text-xs font-black text-slate-400 uppercase tracking-widest">{lang.trend}</h3>
        </div>
        <div className="w-full flex-1 min-h-[300px]" style={{ height: trendHeight }}>
      ...
      <div className="modern-card p-6 flex flex-col items-center border-slate-100 bg-white relative shadow-xl rounded-2xl shrink-0" style={{ height: allocationHeight }}>
