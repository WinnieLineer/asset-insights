            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 h-full items-stretch">
              <Card className="md:col-span-8 lg:col-span-9 modern-card p-4 sm:p-6 relative overflow-hidden bg-white flex flex-col justify-center min-h-[140px]">
                <div className="space-y-2 z-20 relative text-left">
                  <div className="pro-label text-xs sm:text-sm">
                    <Globe className="w-3.5 h-3.5" /> {t.totalValue}
                  </div>
                ...
                <div className="absolute bottom-4 right-4 opacity-5 pointer-events-none">
                  <Wallet className="w-12 h-12 sm:w-20 sm:h-20 text-black" />
                </div>
              </Card>
              ...
              <div className="px-6 py-4 border-b border-slate-50 shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                    <SelectTrigger className="w-[120px] h-8 bg-slate-50 border-slate-200 text-[11px] font-black uppercase rounded-lg"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">{t.allCategories}</SelectItem>
                      {allCategories.map(cat => (<SelectItem key={cat} value={cat}>{t.categoryNames[cat as keyof typeof t.categoryNames] || cat}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
              </div>
                  ...
                  <TableHeader className="relative z-30">
                    <TableRow className="hover:bg-transparent border-none">
                      <TableHead className="sticky top-0 bg-white/95 backdrop-blur-md px-6 h-12 cursor-pointer border-b border-slate-100 z-30" onClick={() => requestSort('active', 'name')}>
                        <div className="flex items-center text-[12px] font-black text-slate-500 uppercase tracking-widest">{t.assetName} <SortIcon config={activeSort} columnKey="name" /></div>
                      </TableHead>
          ...
          <div key={id} className={commonClass} style={wrapperStyle}>
            ...
            <Card className="modern-card bg-white h-full flex flex-col overflow-hidden opacity-80">
              <div className="px-6 py-4 border-b border-slate-50 shrink-0">
                <h3 className="pro-label text-sm"><History className="w-5 h-5" /> {t.closedPositions}</h3>
              </div>
                ...
                <Table className="min-w-[800px] border-separate border-spacing-0">
                   <TableBody>
                    {sortedClosedAssets.map((asset: any) => (
                      <TableRow key={asset.id} className="group hover:bg-slate-50/50 border-slate-50">
                        <TableCell className="px-6 py-4">
                          <div className="font-black text-[13px] text-slate-400 line-through">{asset.name}</div>
                          <div className="text-[11px] font-black text-slate-300 uppercase tracking-[0.1em] mt-0.5">{asset.symbol || asset.category}</div>
                        </TableCell>
                        <TableCell><span className="text-[13px] font-black text-slate-400">{formatNumber(asset.amount)}</span></TableCell>
                        <TableCell className="text-right pr-6"><div className="font-black text-[12px] text-slate-500">{asset.endDate}</div></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
