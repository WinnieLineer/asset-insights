...
export default function AssetInsightsPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-black pb-32 font-sans">
      <header className="fixed top-0 left-0 right-0 border-b border-slate-100 z-[100] bg-white/80 backdrop-blur-xl">
        <div className="max-w-[1600px] mx-auto px-4 h-12 xl:h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-6 h-6 xl:w-8 xl:h-8 bg-black rounded flex items-center justify-center shrink-0 shadow-lg">
              <Activity className="w-3.5 h-3.5 xl:w-4 xl:h-4 text-white" />
            </div>
            <div>
          ...
          <div className="flex items-center gap-2 xl:gap-6 scale-90 xl:scale-100">
            <div className="hidden lg:flex flex-col items-end">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{t.exchangeRate}</span>
              <span className="text-[9px] font-bold text-black flex items-center gap-2 bg-slate-50/80 px-2 py-0.5 rounded-md border border-slate-100">
      ...
      <main className="max-w-[1600px] mx-auto px-4 pt-20 xl:pt-24 space-y-8">
          ...
          {trackingDays === 'custom' && (
            <div className="md:col-span-12 grid grid-cols-2 gap-4 pt-3 border-t border-slate-100 animate-fade-in">
              <div className="space-y-1">
                ...
                <Input type="date" value={customStartDate} onChange={(e) => setCustomStartDate(e.target.value)} className="h-10 bg-white font-bold text-xs rounded-lg border-slate-100" />
              </div>
              <div className="space-y-1">
                <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t.endDate}</Label>
                <Input type="date" value={customEndDate} onChange={(e) => setCustomEndDate(e.target.value)} className="h-10 bg-white font-bold text-xs rounded-lg border-slate-100" />
              </div>
            </div>
          )}
        </section>
        ...
        <section className="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <Card className="modern-card bg-white shadow-xl border-slate-100 rounded-xl overflow-hidden">
              <Tabs defaultValue="active">
                <CardHeader className="px-4 py-3 border-b border-slate-50">
                  <CardTitle className="text-base xl:text-lg font-black tracking-tighter uppercase flex items-center gap-2 mb-3">
                    <BarChart3 className="w-4 h-4 text-primary" /> {t.dashboard}
                  </CardTitle>
          ...
          <div className="xl:col-span-4 space-y-8">
            <AITipCard language={language} assets={assetCalculations.processedAssets} totalTWD={assetCalculations.totalTWD} marketConditions={loading ? "Syncing" : "Stable"} />
